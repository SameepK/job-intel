...
## Appendix A: Industry Best-Practice Diagrams (v8.4.43+)

> Additive section. References: Marty Cagan / Lenny Rachitsky PRD canon, Teresa Torres opportunity-solution trees, Amplitude North Star framework, NN/g Journey Mapping 101 (5 components: Actor, Scenario, Phases, Actions+Mindsets+Emotions, Opportunities). All diagrams are Mermaid per the diagram-as-code skill.

### A.1 Jobs-To-Be-Done Statement

State the user's job in JTBD form before listing requirements:

> When `<situation>`, I want to `<motivation>`, so I can `<expected outcome>`.

| Field | Value |
|-------|-------|
| Situation | {trigger or context} |
| Motivation | {underlying job, not a feature} |
| Expected outcome | {observable result for the user} |
| Hire criteria | {what would cause them to "hire" this product} |
| Fire criteria | {what would cause them to abandon it} |

### A.2 North Star Metric and Driver Tree

```mermaid
flowchart TD
    NS["North Star Metric<br/>{name + target}"]
    D1["Driver 1<br/>{leading indicator}"]
    D2["Driver 2<br/>{leading indicator}"]
    D3["Driver 3<br/>{leading indicator}"]
    I1["Input metric<br/>{controllable}"]
    I2["Input metric<br/>{controllable}"]
    I3["Input metric<br/>{controllable}"]
    I4["Input metric<br/>{controllable}"]
    NS --> D1
    NS --> D2
    NS --> D3
    D1 --> I1
...
### A.3 Opportunity-Solution Tree (Teresa Torres)

```mermaid
flowchart TD
    Outcome["Desired Outcome<br/>{measurable}"]
    Opp1["Opportunity<br/>{user pain or unmet need}"]
    Opp2["Opportunity<br/>{user pain or unmet need}"]
    Sol1["Solution<br/>{idea}"]
    Sol2["Solution<br/>{idea}"]
    Sol3["Solution<br/>{idea}"]
    Exp1["Experiment<br/>{what we will test}"]
    Exp2["Experiment<br/>{what we will test}"]
    Exp3["Experiment<br/>{what we will test}"]
    Outcome --> Opp1
    Outcome --> Opp2
    Opp1 --> Sol1
    Opp1 --> Sol2
...
### A.4 Customer Journey Map (NN/g 5-Element Model)

```mermaid
journey
    title Primary persona journey - {scenario}
    section Discover
      Trigger event: 3: User
      Searches for solution: 4: User
      Evaluates options: 3: User
    section Onboard
      Signs up: 4: User
      Completes setup: 2: User
    section Activate
      Reaches first value: 5: User
      Invites teammate: 4: User
    section Retain
      Returns weekly: 5: User
      Hits friction point: 2: User
    section Advocate
      Recommends product: 5: User
```

| Phase | User actions | Mindset / questions | Emotion (1-5) | Opportunity |
|-------|--------------|---------------------|---------------|-------------|
| Discover | {actions} | {what they think} | {score} | {improvement idea} |
| Onboard | {actions} | {what they think} | {score} | {improvement idea} |
| Activate | {actions} | {what they think} | {score} | {improvement idea} |
| Retain | {actions} | {what they think} | {score} | {improvement idea} |
| Advocate | {actions} | {what they think} | {score} | {improvement idea} |

### A.5 Now / Next / Later Outcome Roadmap

```mermaid
flowchart LR
    subgraph Now["Now (current quarter)"]
        N1["Theme: {outcome}"]
        N2["Theme: {outcome}"]
    end
...
### A.6 Hypotheses and Falsifiers

| ID | Belief | Test | Falsifier (will disprove if) | Result |
|----|--------|------|------------------------------|--------|
| H-1 | {what we believe} | {experiment, metric, sample} | {threshold or signal} | {pending / supported / disproved} |
| H-2 | {what we believe} | {experiment, metric, sample} | {threshold or signal} | {pending / supported / disproved} |

### A.7 Instrumentation Plan

| Event | Trigger | Properties | Owner | Tied to North Star driver |
|-------|---------|------------|-------|----------------------------|
| {event_name} | {when fired} | {props} | {team} | {D1/D2/D3} |
| {event_name} | {when fired} | {props} | {team} | {D1/D2/D3} |


## Appendix B: Rich Visual Diagrams (v8.4.43+)

### B.1 Persona x Jobs Mindmap
...
### B.2 Outcome Trajectory (xychart)

```mermaid
xychart-beta
  title "North Star trajectory vs target"
  x-axis [Q1, Q2, Q3, Q4]
  y-axis "North Star metric" 0 --> 100
  bar [20, 35, 55, 80]
  line [25, 50, 70, 90]
```

### B.3 Effort Allocation (pie)

```mermaid
pie showData
  title Effort allocation by theme
  "Theme: Acquisition" : 30
  "Theme: Activation" : 25
  "Theme: Retention" : 25
  "Theme: Platform / debt" : 20
```

### B.4 Discovery -> Delivery Flow (styled)

```mermaid
flowchart LR
  D[Discovery]:::disc --> H[Hypothesis]:::hyp --> E[Experiment]:::exp --> M[Measure]:::meas --> L[Learn]:::learn
  L -->|persevere| Build[Build]:::build
  L -->|pivot| D
  Build --> Ship[Ship]:::ship --> M
  classDef disc fill:#e3f2fd,stroke:#1976d2,color:#0d47a1
  classDef hyp fill:#fff3e0,stroke:#f57c00,color:#e65100
  classDef exp fill:#f3e5f5,stroke:#7b1fa2,color:#4a148c
  classDef meas fill:#e8f5e9,stroke:#388e3c,color:#1b5e20
  classDef learn fill:#fce4ec,stroke:#c2185b,color:#880e4f
  classDef build fill:#e0f7fa,stroke:#0097a7,color:#006064
  classDef ship fill:#fffde7,stroke:#fbc02d,color:#f57f17
```
